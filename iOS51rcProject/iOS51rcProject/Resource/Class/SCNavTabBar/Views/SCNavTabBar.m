...
- (void)viewConfig
{
    _navgationTabBar = [[UIScrollView alloc] initWithFrame:CGRectMake(DOT_COORDINATE, DOT_COORDINATE, SCREEN_WIDTH, SCNAV_BAR_HEIGHT)];
    _navgationTabBar.showsHorizontalScrollIndicator = NO;
    _navgationTabBar.backgroundColor = [UIColor whiteColor];
    UIView *viewBottomSeperate = [[UIView alloc] initWithFrame:CGRectMake(0, VIEW_BY(_navgationTabBar), VIEW_W(_navgationTabBar), 1)];
...
{
    _line = [[UIView alloc] initWithFrame:CGRectMake(width * 0.2f, SCNAV_BAR_HEIGHT - 3.0f, width * 0.6f, 3.0f)];
    _line.backgroundColor = (self.isCompany ? CPNAVBARCOLOR : NAVBARCOLOR);
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        button.frame = CGRectMake(buttonX, DOT_COORDINATE, [widths[index] floatValue], SCNAV_BAR_HEIGHT);
        [button setTitle:_itemTitles[index] forState:UIControlStateNormal];
        [button.titleLabel setFont:DEFAULTFONT];
        [button setTitleEdgeInsets:UIEdgeInsetsMake(0, 3, 0, 3)];
        if (index == 0) {
            [button setTitleColor:(self.isCompany ? CPNAVBARCOLOR : NAVBARCOLOR) forState:UIControlStateNormal];
        }
        else {
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        float fltWidth = [_itemsWidth[currentItemIndex] floatValue];
        _line.frame = CGRectMake(button.frame.origin.x + fltWidth * 0.2f, _line.frame.origin.y, fltWidth * 0.6f, _line.frame.size.height);
    }];
}
