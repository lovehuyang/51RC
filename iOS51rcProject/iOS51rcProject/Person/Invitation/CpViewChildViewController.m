...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    WKLabel *lbCompany = [[WKLabel alloc] initWithFixedHeight:CGRectMake(VIEW_BX(imgLogo) + 15, VIEW_Y(imgLogo), maxWidth, 25) content:[data objectForKey:@"cpName"] size:BIGGERFONTSIZE color:nil];
    [cell.contentView addSubview:lbCompany];
    
    WKLabel *lbDate = [[WKLabel alloc] initWithFixedHeight:CGRectMake(VIEW_X(lbCompany), VIEW_BY(lbCompany), 200, 25) content:[NSString stringWithFormat:@"查看时间：%@", [Common stringFromDateString:[data objectForKey:@"adddate"] formatType:@"MM-dd HH:mm"]] size:DEFAULTFONTSIZE color:TEXTGRAYCOLOR];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    jobCtrl.companyId = [data objectForKey:@"cpID"];
