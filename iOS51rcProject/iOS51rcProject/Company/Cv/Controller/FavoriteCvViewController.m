...
@interface FavoriteCvViewController ()<UITableViewDelegate, UITableViewDataSource, NetWebServiceRequestDelegate>
...
@implementation FavoriteCvViewController
...
- (void)getData {
    NetWebServiceRequest *request = [NetWebServiceRequest serviceRequestUrlCp:@"GetCpFavourCv" Params:[NSDictionary dictionaryWithObjectsAndKeys:CAMAINID, @"caMainID", CAMAINCODE, @"Code", [NSString stringWithFormat:@"%ld", self.page], @"intPageNo", nil] viewController:self];
    [request setTag:1];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        cell = [[WKCvTableViewCell alloc] initWithListType:2 reuseIdentifier:@"cell" viewController:self];
    }
    for (UIView *view in cell.contentView.subviews) {
        [view removeFromSuperview];
    }
    [cell fillCvInfo:[NSString stringWithFormat:@"所属职位：%@", [data objectForKey:@"JobName"]] gender:[data objectForKey:@"Gender"] name:[[data objectForKey:@"paName"] stringByReplacingOccurrencesOfString:@"$$##" withString:@""] relatedWorkYears:[data objectForKey:@"RelatedWorkYears"] age:[data objectForKey:@"Age"] degree:[data objectForKey:@"DegreeName"] livePlace:[data objectForKey:@"LivePlaceName"] loginDate:[data objectForKey:@"adddate"] mobileVerifyDate:[data objectForKey:@"MobileVerifyDate"] paPhoto:[data objectForKey:@"PaPhoto"] online:[data objectForKey:@"IsOnline"] paMainId:[data objectForKey:@"paMainID"] cvMainId:[data objectForKey:@"cvMainID"]];
    [cell setJobId:[data objectForKey:@"JobID"]];
    return cell;
}
