...
@interface ChatCpViewController ()<WKNavigationDelegate, WKScriptMessageHandler, UITextFieldDelegate, UIScrollViewDelegate, NetWebServiceRequestDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, MLImageCropDelegate>
...
@implementation ChatCpViewController
...
- (void)viewDidLoad {
    ...
    [config.userContentController addScriptMessageHandler:self name:@"resumeView"];
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://%@/chatonline/cplogmobile?cv=%@&caMainId=%@&code=%@&app=1", [USER_DEFAULT valueForKey:@"subsite"], self.cvMainId, CAMAINID, CAMAINCODE]];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    ...
    else if ([message.name isEqualToString:@"resumeView"]) {
        CvDetailViewController *cvDetailCtrl = [[CvDetailViewController alloc] init];
        cvDetailCtrl.cvMainId = self.cvMainId;
...
- (void)uploadPhoto:(NSString *)dataPhoto {
    NetWebServiceRequest *request = [NetWebServiceRequest serviceRequestUrlCp:@"UploadChatFile" Params:[NSDictionary dictionaryWithObjectsAndKeys:dataPhoto, @"stream", CAMAINID, @"caMainID", CAMAINCODE, @"code", [USER_DEFAULT objectForKey:@"provinceId"], @"subsiteID", nil] viewController:self];
    [request setTag:2];
...
- (void)sendMessage {
    NetWebServiceRequest *request = [NetWebServiceRequest serviceRequestUrlCp:@"SendMessage" Params:[NSDictionary dictionaryWithObjectsAndKeys:CAMAINID, @"caMainId", CAMAINCODE, @"code", self.cvMainId, @"cvMainId", self.jobId, @"jobId", self.message, @"message", self.messageType, @"messageType", nil] viewController:nil];
    [request setTag:1];
    [request setDelegate:self];
    [request startAsynchronous];
    self.runningRequest = request;
...
- (void)netRequestFinished:(NetWebServiceRequest *)request
    ...
    if (request.tag == 1) {
        if ([result isEqualToString:@"-1"]) {
            [self.view makeToast:@"您的企业还未认证，不能和求职者进行对话，请先到电脑端完成企业认证"];
            return;
        }
        [self.webView evaluateJavaScript:@"fillCpLog();" completionHandler:nil];
        [self.view endEditing:YES];
        [self emotionCancel];
    }
